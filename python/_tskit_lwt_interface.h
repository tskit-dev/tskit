 ...
 */
typedef struct {
  PyObject_HEAD tsk_table_collection_t *tables;
} LightweightTableCollection;

static void handle_tskit_error(int err) {
  PyErr_SetString(PyExc_ValueError, tsk_strerror(err));
}

static PyObject *make_Py_Unicode_FromStringAndLength(const char *str,
                                                     size_t length) {
  PyObject *ret = NULL;
 ...
 */
static PyObject *get_table_dict_value(PyObject *dict, const char *key_str,
                                      bool required) {
  PyObject *ret = NULL;
...
static PyArrayObject *table_read_column_array(PyObject *input, int npy_type,
                                              size_t *num_rows,
                                              bool check_num_rows) {
  ...
  array = (PyArrayObject *)PyArray_FROMANY(input, npy_type, 1, 1,
                                           NPY_ARRAY_IN_ARRAY);
  if (array == NULL) {
    goto out;
  }
  ...
  if (check_num_rows) {
    if (*num_rows != (size_t)shape[0]) {
      PyErr_SetString(PyExc_ValueError,
                      "Input array dimensions must be equal.");
      goto out;
    }
  } else {
...
static PyArrayObject *table_read_offset_array(PyObject *input, size_t *num_rows,
                                              size_t length,
                                              bool check_num_rows) {
  PyArrayObject *ret = NULL;
  PyArrayObject *array = NULL;
  ...
  array = (PyArrayObject *)PyArray_FROMANY(input, NPY_UINT32, 1, 1,
                                           NPY_ARRAY_IN_ARRAY);
  if (array == NULL) {
    goto out;
  }
...
static int parse_individual_table_dict(tsk_individual_table_t *table,
                                       PyObject *dict, bool clear_table) {
  int err;
  int ret = -1;
  ...
  /* Pull out the arrays */
  flags_array =
      table_read_column_array(flags_input, NPY_UINT32, &num_rows, false);
  if (flags_array == NULL) {
    goto out;
  }
  ...
  if ((parents_input == Py_None) != (parents_offset_input == Py_None)) {
    PyErr_SetString(PyExc_TypeError,
                    "parents and parents_offset must be specified together");
    goto out;
  }
  if (parents_input != Py_None) {
    parents_array = table_read_column_array(parents_input, NPY_INT32,
                                            &parents_length, false);
  ...
  if (metadata_schema_input != Py_None) {
    metadata_schema = parse_metadata_schema_arg(metadata_schema_input,
                                                &metadata_schema_length);
    if (metadata_schema == NULL) {
      goto out;
    }
    err = tsk_individual_table_set_metadata_schema(table, metadata_schema,
                                                   metadata_schema_length);
    if (err != 0) {
  ...
  err = tsk_individual_table_append_columns(
      table, num_rows, PyArray_DATA(flags_array), location_data,
      location_offset_data, parents_data, parents_offset_data, metadata_data,
...
static int parse_node_table_dict(tsk_node_table_t *table, PyObject *dict,
                                 bool clear_table) {
  int err;
  int ret = -1;
  ...
  time_array =
      table_read_column_array(time_input, NPY_FLOAT64, &num_rows, true);
  if (time_array == NULL) {
    goto out;
  }
  ...
  if (metadata_schema_input != Py_None) {
    ...
    err = tsk_node_table_set_metadata_schema(table, metadata_schema,
                                             metadata_schema_length);
    if (err != 0) {
      handle_tskit_error(err);
      goto out;
  ...
  err = tsk_node_table_append_columns(
      table, num_rows, PyArray_DATA(flags_array), PyArray_DATA(time_array),
      population_data, individual_data, metadata_data, metadata_offset_data);
  if (err != 0) {
    handle_tskit_error(err);
    goto out;
  }
...
static int parse_edge_table_dict(tsk_edge_table_t *table, PyObject *dict,
                                 bool clear_table) {
  int ret = -1;
  int err;
  size_t num_rows = 0;
  ...
  /* Create the arrays */
  left_array =
      table_read_column_array(left_input, NPY_FLOAT64, &num_rows, false);
  if (left_array == NULL) {
    goto out;
  }
  right_array =
      table_read_column_array(right_input, NPY_FLOAT64, &num_rows, true);
  if (right_array == NULL) {
    goto out;
  }
  parent_array =
      table_read_column_array(parent_input, NPY_INT32, &num_rows, true);
  if (parent_array == NULL) {
    goto out;
  }
  child_array =
      table_read_column_array(child_input, NPY_INT32, &num_rows, true);
  if (child_array == NULL) {
    goto out;
  }
  if ((metadata_input == Py_None) != (metadata_offset_input == Py_None)) {
  ...
  if (metadata_schema_input != Py_None) {
    ...
    err = tsk_edge_table_set_metadata_schema(table, metadata_schema,
                                             metadata_schema_length);
    if (err != 0) {
      handle_tskit_error(err);
      goto out;
  ...
  err = tsk_edge_table_append_columns(
      table, num_rows, PyArray_DATA(left_array), PyArray_DATA(right_array),
      PyArray_DATA(parent_array), PyArray_DATA(child_array), metadata_data,
      metadata_offset_data);
  if (err != 0) {
    handle_tskit_error(err);
    goto out;
...
static int parse_migration_table_dict(tsk_migration_table_t *table,
                                      PyObject *dict, bool clear_table) {
  int err;
  int ret = -1;
  size_t num_rows;
  ...
  source_array =
      table_read_column_array(source_input, NPY_INT32, &num_rows, true);
  ...
  if (metadata_schema_input != Py_None) {
    ...
    err = tsk_migration_table_set_metadata_schema(table, metadata_schema,
                                                  metadata_schema_length);
    if (err != 0) {
      handle_tskit_error(err);
      goto out;
  ...
  err = tsk_migration_table_append_columns(
      table, num_rows, PyArray_DATA(left_array), PyArray_DATA(right_array),
      PyArray_DATA(node_array), PyArray_DATA(source_array),
...
static int parse_site_table_dict(tsk_site_table_t *table, PyObject *dict,
                                 bool clear_table) {
  int err;
  int ret = -1;
  size_t num_rows = 0;
  ...
  if (metadata_schema_input != Py_None) {
    ...
    err = tsk_site_table_set_metadata_schema(table, metadata_schema,
                                             metadata_schema_length);
    if (err != 0) {
      handle_tskit_error(err);
      goto out;
  ...
  err = tsk_site_table_append_columns(
      table, num_rows, PyArray_DATA(position_array),
      PyArray_DATA(ancestral_state_array),
      PyArray_DATA(ancestral_state_offset_array), metadata_data,
      metadata_offset_data);
  if (err != 0) {
    handle_tskit_error(err);
    goto out;
...
static int parse_mutation_table_dict(tsk_mutation_table_t *table,
                                     PyObject *dict, bool clear_table) {
  int err;
  int ret = -1;
  size_t num_rows = 0;
  ...
  if (metadata_schema_input != Py_None) {
    ...
    err = tsk_mutation_table_set_metadata_schema(table, metadata_schema,
                                                 metadata_schema_length);
    if (err != 0) {
      handle_tskit_error(err);
      goto out;
  ...
  err = tsk_mutation_table_append_columns(
      table, num_rows, PyArray_DATA(site_array), PyArray_DATA(node_array),
      parent_data, time_data, PyArray_DATA(derived_state_array),
      PyArray_DATA(derived_state_offset_array), metadata_data,
      metadata_offset_data);
  if (err != 0) {
    handle_tskit_error(err);
    goto out;
...
static int parse_population_table_dict(tsk_population_table_t *table,
                                       PyObject *dict, bool clear_table) {
  int err;
  int ret = -1;
  size_t num_rows, metadata_length;
  ...
  if (metadata_schema_input != Py_None) {
    ...
    err = tsk_population_table_set_metadata_schema(table, metadata_schema,
                                                   metadata_schema_length);
    if (err != 0) {
      handle_tskit_error(err);
      goto out;
  ...
  err = tsk_population_table_append_columns(
      table, num_rows, PyArray_DATA(metadata_array),
      PyArray_DATA(metadata_offset_array));
  if (err != 0) {
    handle_tskit_error(err);
    goto out;
  }
...
static int parse_provenance_table_dict(tsk_provenance_table_t *table,
                                       PyObject *dict, bool clear_table) {
  int err;
  int ret = -1;
  ...
  err = tsk_provenance_table_append_columns(
      table, num_rows, PyArray_DATA(timestamp_array),
      PyArray_DATA(timestamp_offset_array), PyArray_DATA(record_array),
      PyArray_DATA(record_offset_array));
  if (err != 0) {
    handle_tskit_error(err);
    goto out;
  }
...
static int parse_table_collection_dict(tsk_table_collection_t *tables,
  ...
  if (value != Py_None) {
    ...
    err = tsk_table_collection_set_metadata_schema(tables, metadata_schema,
                                                   metadata_schema_length);
    if (err != 0) {
      handle_tskit_error(err);
      goto out;
...
static int write_table_arrays(tsk_table_collection_t *tables, PyObject *dict) {
  struct table_col {
    const char *name;
    void *data;
    npy_intp num_rows;
  ...
  struct table_col node_cols[] = {
      ...
      {"flags", (void *)tables->nodes.flags, tables->nodes.num_rows,
       NPY_UINT32},
      ...
      {"individual", (void *)tables->nodes.individual, tables->nodes.num_rows,
       NPY_INT32},
      {"metadata", (void *)tables->nodes.metadata,
       tables->nodes.metadata_length, NPY_INT8},
  ...
  struct table_col edge_cols[] = {
      ...
      {"right", (void *)tables->edges.right, tables->edges.num_rows,
       NPY_FLOAT64},
      {"parent", (void *)tables->edges.parent, tables->edges.num_rows,
       NPY_INT32},
      {"child", (void *)tables->edges.child, tables->edges.num_rows, NPY_INT32},
      {"metadata", (void *)tables->edges.metadata,
       tables->edges.metadata_length, NPY_INT8},
  ...
  struct table_col site_cols[] = {
      ...
      {"metadata", (void *)tables->sites.metadata,
       tables->sites.metadata_length, NPY_INT8},
  ...
  for (j = 0; j < sizeof(table_descs) / sizeof(*table_descs); j++) {
    ...
    while (col->name != NULL) {
      array =
          PyArray_SimpleNewFromData(1, &col->num_rows, col->type, col->data);
      if (array == NULL) {
        goto out;
      }
...
static PyObject *dump_tables_dict(tsk_table_collection_t *tables) {
  PyObject *ret = NULL;
  PyObject *dict = NULL;
  PyObject *val = NULL;
  int err;
  ...
  if (tables->metadata_schema_length > 0) {
    val = make_Py_Unicode_FromStringAndLength(tables->metadata_schema,
...
LightweightTableCollection_check_state(LightweightTableCollection *self) {
  ...
  if (self->tables == NULL) {
    PyErr_SetString(PyExc_SystemError,
                    "LightweightTableCollection not initialised");
    ret = -1;
  }
  return ret;
}
...
static int LightweightTableCollection_init(LightweightTableCollection *self,
...
static PyObject *
LightweightTableCollection_fromdict(LightweightTableCollection *self,
                                    PyObject *args) {
  int err;
  PyObject *ret = NULL;
  PyObject *dict = NULL;
...
static PyTypeObject LightweightTableCollectionType = {
    PyVarObject_HEAD_INIT(
        NULL, 0) "_msprime.LightweightTableCollection", /* tp_name */
...
static int register_lwt_class(PyObject *module) {
