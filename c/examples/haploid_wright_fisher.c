...
simulate(tsk_tbl_collection_t *tables, int N, int T, int simplify_interval, gsl_rng *rng)
{
    ...
    for (j = 0; j < N; j++) {
        parents[j] = tsk_node_tbl_add_row(tables->nodes, TSK_NODE_IS_SAMPLE, T,
    ...
    for (t = T - 1; t >= 0; t--) {
        ...
        for (j = 0; j < N; j++) {
            child = tsk_node_tbl_add_row(tables->nodes, TSK_NODE_IS_SAMPLE, t,
                    TSK_NULL, TSK_NULL, NULL, 0);
            check_error(child);
            ...
            ret = tsk_edge_tbl_add_row(tables->edges, 0, breakpoint, left_parent, child);
            check_error(ret);
            ret = tsk_edge_tbl_add_row(tables->edges, breakpoint, 1, right_parent, child);
            check_error(ret);
            children[j] = child;
        }
        parents = children;
        ...
        if (t % simplify_interval == 0) {
            ret = tsk_tbl_collection_sort(tables, 0, 0); /* FIXME; should take position. */
            check_error(ret);
            ret = tsk_tbl_collection_simplify(tables, parents, N, 0, NULL);
            check_error(ret);
            for (j = 0; j < N; j++) {
                parents[j] = j;
            }
